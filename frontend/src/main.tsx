...
function App() {
  ...
  const scan = async () => { try { const r=await fetch("/api/catalog/scan", {method:"POST",headers:{"content-type":"application/json"},body:"{}"}); if (!r.ok) throw new Error(); await refresh(); message.success("目录扫描完成"); } catch { message.error("扫描失败，请检查后端和数据路径"); } };
  useEffect(refresh, []);
