...
- Encoding a video feature requires an integer `fps` passed to
  ...
  non-video path accepts (and, per `shared/lerobot_io.py`, relies on) `fps`
  being a float at runtime despite its `int` type hint.
...
- A decoded video frame -- `dataset[i][<video_key>]` -- is a `torch.Tensor`
  ...
  This is *not* the `(H, W, 3)` uint8 `0-255` layout `shared/lerobot_io.py`'s
